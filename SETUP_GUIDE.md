...
## Phase 9 - Verify, screenshot, publish

Functional check at the hosting URL:

- Ratings & Trends: rating lines from 1950, decade bars, top-rated table
  with posters
- Genre Analysis: genre chips drive the output chart; rating comparison
  renders
- People Analytics: director ranking; row click loads a filmography
- Search & Explore: search narrows instantly; title click opens the
  credits panel with poster
- TMDB attribution notice visible in the footer on every view

Blog assets (structure in `docs/blog-post-skeleton.md`):

- Screenshots of all four views, footer notice visible where possible
- Workspace item list next to the running app - one Fabric App item, no
  report artefacts, that contrast is the story
- Notebook 01 threshold-count output

Final push, then write the article.
...
| Symptom | Cause | Fix |
|---|---|---|
| `npm install` fails at node-gyp/keytar | Missing Xcode CLT | `xcode-select --install`, retry |
| `@entity()` underlined in VS Code | Built-in TS version active | Select workspace TypeScript version (Phase 1) |
| TS1238/TS1240 decorator errors | `experimentalDecorators` added to tsconfig | Remove it; this SDK uses standard TC39 decorators |
| `@microsoft/rayfin` 404 on install | Package does not exist | Depend on rayfin-core / rayfin-client / rayfin-cli individually |
| `rayfin up` auth error | Not signed in or workload disabled | `npx rayfin login`; confirm Fabric Apps (preview) with the admin |
| Deployed app login broken | `FABRIC_AUTH_ENABLED` false at deploy | Set true in `rayfin/.env`, `npx rayfin up` again |
| Entity missing from GraphQL API | Not registered in `rayfin/data/schema.ts` | Register, `npx rayfin up db apply` |
| Notebook 03 cannot connect | Wrong server/database or no item permission | Re-copy SQL endpoint from the item; check workspace role |
| Notebook 03 unknown table name | Generated names differ from entity names | `SELECT name FROM sys.tables`; adjust upsert targets |
| Notebook 03 FK violation | Load order changed | Parents first: Title and Person before Principal |
| Repeated 429s in Notebook 02 | Too many workers | Lower `MAX_WORKERS` to 4 |
| Frontend "Could not load data" locally | Vite env file missing or stale | `npx rayfin env`, restart `npm run dev` |
| Charts render, tables empty | Notebook 02/03 partial run | Re-run 02 (resumes), then 03 |
| `staticapp deploy` rejected on size | dist over 100MB compressed | Check for accidental assets in `dist/`; sourcemaps are already off |
