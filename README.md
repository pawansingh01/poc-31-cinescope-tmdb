...
- Scheduled refresh pipeline (monthly) instead of manual notebook runs, with TMDB delta detection via /movie/changes
- Write-back scenario: user watchlists or ratings via @role-protected entities, exercising Rayfin auth properly
- Fabric IQ agent over the curated film dataset
- Widen scope: writers and composers in Principal, languages and countries facets
